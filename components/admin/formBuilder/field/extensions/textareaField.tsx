...
const Field: FC<Props> = ({ value: field, onDelete }) => {
  const fieldExtension = field.getExtension<"textarea">();
  return (
    <div className="w-full mt-4 flex flex-col items-stretch">
      <div className="flex text-sm">
        ...
        <div className="w-1/2 pl-2">
          <h4 className="text-sm text-gray-600">Rows</h4>
          <input
            className="border border-gray-200 py-2 px-4 w-full rounded-sm"
            type={"number"}
            value={fieldExtension.rows}
            onChange={e => fieldExtension.setRows(e.target.value)}
          />
        </div>
      </div>
...
export const TextAreaField = observer(Field);
