...
import { AdminObservationDefinitionListViewModel } from "./listViewModel";
...
const ObservationDefinitionList = () => {
  const router = useRouter();
  const { t } = useTranslation();
  const { observationDefinitionService } = useServices();
  const { setUrl, query, resetUrl } = useUrlParams();

  const [viewModel] = useState<AdminObservationDefinitionListViewModel>(() => {
    const model = new AdminObservationDefinitionListViewModel(
      ...
      {() => (
        <div>
          <div className="flex items-center flex-wrap mb-4 gap-2">
            ...
            <Link href={"/admin/observation_definitions/create"} passHref>
              <AddButton />
            </Link>
          </div>
          ...
          <Table
            ...
            onView={record =>
              router.push(`/admin/observation_definitions/${record.id}/view`)
            }
          ...
          <ConfirmDialog
            ...
            onYes={(record: ObservationDefinition) =>
...
export default ObservationDefinitionList;
