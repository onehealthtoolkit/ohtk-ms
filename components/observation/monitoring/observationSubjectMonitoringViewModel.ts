...
import { ObservationSubjectMonitoringDetail } from "lib/services/observation/observation";

export class ObservationSubjectMonitoringViewModel extends BaseViewModel {
  data: ObservationSubjectMonitoringDetail =
    {} as ObservationSubjectMonitoringDetail;
  id: string;
  ...
  async fetch() {
    this.isLoading = true;
    const data = (
      await this.observationService.getObservationSubjectMonitoring(this.id)
    ).data;
    if (data) {
      runInAction(() => {
        this.data = data;
