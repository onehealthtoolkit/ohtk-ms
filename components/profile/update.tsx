...
import { ProfileUpdateViewModel } from "components/profile/updateViewModel";
import {
  ErrorText,
  Field,
  FieldGroup,
...
const ProfileUpdate = () => {
  ...
  const [viewModel] = useState(
    () => new ProfileUpdateViewModel(services.profileService)
  );
            ...
            <Field $size="half" className="flex flex-col items-center">
              <div className="w-48 h-48 sm:w-36 sm:h-36">
                {viewModel.imageUrl ? (
                  <img
                    src={`${
                      viewModel.imageUrl.indexOf("blob") === -1
              ...
              <input
                ...
                onChange={evt => {
                  if (evt.target.files?.length) {
                    ...
                    viewModel.imageUrl = URL.createObjectURL(
                      evt.target.files[0]
                    );
                  }
                }}
